...
export function CommandsView() {
  ...
  return <section className="space-y-4" aria-label="Custom commands">
    ...
    <form className="grid gap-3 md:grid-cols-2" onSubmit={e => { e.preventDefault(); void save() }}>
      ...
      <label className="text-sm">Specialist<select className={field} value={draft.role} onChange={e => setDraft({ ...draft, role: e.target.value })}>{['explore', 'plan', 'design', 'build', 'debug', 'review', 'plan-review', 'simplify', 'testing', 'research'].map(r => <option key={r}>{r}</option>)}</select></label>
