...
**15** sau hậu kiểm).
### 6.27 Vòng 22 (đợt 3, vòng kiểm độc lập) — BUG-67 (báo động sai vì backtick) và BUG-70 (khối `Bằng chứng` đếm trùng) ĐÃ SỬA; BUG-71 (rác tệp do phép dò tự ghi) ghi nhận
...
**BUG-71 — mức Thấp — ghi nhận, CHƯA SỬA (vòng sau).** Phép dò của cổng (`probe_workspace`) tự ghi kết quả rồi bị chính tầng ghi bằng chứng của worker bắt thêm một lần: mỗi lượt `needs_probe` sinh **hai** tệp trong `.generated_artifacts/captures/evidence/<sid8>/` — bản đọc được `<sid8>_<step>_changes.txt` (đúng ý) và bản `.diff` tên `<sid8>_000_<sid8>_<step>_changes.txt.diff` (step `000` vì lượt ghi nội bộ không mang identity, và không mảnh cổng nào trỏ tới). Thấy ở `4987d659` và sau khi sửa ở `1c65d2e7`. Cách sửa gợi ý: cho `probe_workspace` truyền identity của lượt/bước (hoặc đánh dấu lần ghi này là nội bộ) để phép dò không sinh thêm tệp.
