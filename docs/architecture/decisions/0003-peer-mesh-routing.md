...
| 7 | **Con KHÔNG tự sinh anh em; cây con phẳng một tầng** | không có `spawn_peer`; `max_depth = 1` | T5, D-11 |
...
4. **Giao hàng có định tuyến.** `deliverTo` nhận tối đa `PEER_DELIVER_MAX = 4` địa chỉ; địa chỉ hiểu cả `role:<vai>` lẫn `main`. Mục tiêu chưa tồn tại vẫn nhận được: mỗi giây một lần trong `PEER_TARGET_GRACE_SECONDS = 20`, vì anh em có thể được sinh ngay sau người gửi.
...
7. **Cây con phẳng một tầng.** Con chỉ "nhìn thấy nhau" ở mức đọc (`peer_read`), đợi (`await_children`) và nhận giao hàng. Mọi con đều cùng cha, nên `max_depth = 1`: không có `lineage` sâu, không có vòng lặp cha–con, và mọi trần ở mục 1 là trần của cả cây.
