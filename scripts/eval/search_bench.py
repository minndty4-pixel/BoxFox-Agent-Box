...
DEFAULT_QUERIES = Path(__file__).resolve().parent / 'search_queries.jsonl'
...
def run_query(query: dict, config: str) -> dict:
    ...
    try:
        rows = _perform_search(query['text'], spec)
        error = None
        error_code = None
...
def _perform_search(text: str, spec: dict) -> list[dict]:  # pragma: no cover - đường sống
    """Đường tìm thật. Gói nguồn LUÔN tắt (8.7 chạy không gói nguồn)."""
...
def judge_pool(pool: list[dict], out_dir: Path, *, runner=None) -> dict[str, int]:
    """Chấm gộp (pooled) qua `judge.JudgeRunner`; nhãn đã có thì dùng lại, không gọi lại."""
    ...
    if cache_path.exists():
        for line in cache_path.read_text(encoding='utf-8').splitlines():
            if line.strip():
                item = json.loads(line)
                grades[item['label']] = int(item['grade'])
    ...
    with cache_path.open('a', encoding='utf-8') as handle:
        for item in missing:
            ...
            handle.write(json.dumps({'label': item['label'], 'grade': grade},
                                    ensure_ascii=False) + '\n')
    return grades
